...
## 2.5. Re-deliver inbox-from-brain в проекты (для свежего компа / после `git pull`)

Копии заявок в `<проект>/docs/inbox-from-brain/` — **не закоммичены в проектах** (по правилу «не лезть в код-проекты из brain_matrica»). На новом компе после `git pull` соседних проектов их **не будет** — но canonical в `brain_matrica/dispatch/items/` есть всегда.

Проверь и при необходимости разложи:

```bash
for proj in MatricaRMZ GONBA setka; do
  test -d "../$proj/docs/inbox-from-brain" || mkdir -p "../$proj/docs/inbox-from-brain"
done
```

Для каждой 📤 sent заявки в `dispatch/INDEX.md`:
1. Прочти `dispatch/items/NNNN-*.md`.
2. Проверь target.
3. Если копии в `<target>/docs/inbox-from-brain/NNNN-*.md` **нет** или содержимое устарело — положи актуальный бриф туда.
4. Если у проекта нет даже README — создай его по образцу (см. PROTOCOL.md секцию про inbox-from-brain).

После этого в проектных сессиях `/start` найдёт заявки. Закоммитят их **проектные сессии**, не brain_matrica.
