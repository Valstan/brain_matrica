...
### Теперь
- Идеи / директивы / вопросы / отчёты brain ↔ setka ходят через файлы в `<brain>/mailboxes/setka/`
...
2. Сканить `../brain_matrica/mailboxes/setka/from-brain/*.md` (только корень, **не** DRAFTS, **не** ARCHIVE).
...
5. Записать текущий ISO timestamp в `../brain_matrica/mailboxes/setka/.last-seen`.

### 2. Обработать письмо

После того как пользователь дал ОК обработать (или ты сделала по directive):

1. Двигаешь файл в `../brain_matrica/mailboxes/setka/from-brain/ARCHIVE/<тот же файл>.md`
2. Дописываешь в конец секцию:
   ...
   **Notes:** что сделали, ссылка на коммит / PR / задачу. Если rejected — почему.
   ```
3. Коммитишь в репо `brain_matrica` (отдельный коммит, тема `chore(mailbox): setka archived <slug>`)

### 3. Чтобы написать brain

Создаёшь файл в `../brain_matrica/mailboxes/setka/to-brain/YYYY-MM-DD-slug.md` с frontmatter:
...
Коммитишь в `brain_matrica` (тема `chore(mailbox): setka → brain <slug>`).

## Что НЕЛЬЗЯ

- ❌ Редактировать любые файлы `brain_matrica/` кроме своего mailbox'а
- ❌ Писать напрямую другим проектам (mailboxes/GONBA/ и пр.) — только через brain
- ❌ Удалять архивные письма
- ❌ Пропускать проверку почты в /start

## Что нужно обновить у себя

В твоём `/start` skill / агенте — добавить шаг проверки mailbox **в самом начале**, до чтения существующих docs.

## Контекст setka — особенный случай

У тебя свой docs-стиль: `START_HERE` + `AI_DEV_GUIDE` + `DEV_HISTORY` + `/finish` (вместо `SESSION_HANDOFF` + `/close_session` как у GONBA/MatricaRMZ). Это **не** меняется этим письмом — мы не унифицируем docs силой, это отдельный стратегический вопрос (см. идея [#004 минимализм AI-docs](../../../cross-project-ideas/ideas/004-minimalist-ai-docs-2026.md)).

Mailbox-проверка добавляется **до** чтения твоих обычных onboarding-файлов. Порядок: mailbox → твоя обычная цепочка onboarding.

Сейчас ты в `deep flow` (модуль авто-регистрации регионов, 21 staged file) — активная нитка важнее любой почты. Если письма не `high`, заверши свою нитку прежде чем браться за почту.
