...
### Теперь
- Идеи / директивы / вопросы / отчёты brain ↔ MatricaRMZ ходят через файлы в `<brain>/mailboxes/MatricaRMZ/`
...
2. Сканить `../brain_matrica/mailboxes/MatricaRMZ/from-brain/*.md` (только корень, **не** DRAFTS, **не** ARCHIVE).
...
5. Записать текущий ISO timestamp в `../brain_matrica/mailboxes/MatricaRMZ/.last-seen`.

### 2. Обработать письмо

После того как пользователь дал ОК обработать (или ты сделала по directive):

1. Двигаешь файл в `../brain_matrica/mailboxes/MatricaRMZ/from-brain/ARCHIVE/<тот же файл>.md`
2. Дописываешь в конец секцию:
   ...
   **Notes:** что сделали, ссылка на коммит / PR / задачу. Если rejected — почему.
   ```
3. Коммитишь в репо `brain_matrica` (отдельный коммит, тема `chore(mailbox): MatricaRMZ archived <slug>`)

### 3. Чтобы написать brain

Создаёшь файл в `../brain_matrica/mailboxes/MatricaRMZ/to-brain/YYYY-MM-DD-slug.md` с frontmatter:
...
Коммитишь в `brain_matrica` (тема `chore(mailbox): MatricaRMZ → brain <slug>`).
...
Сейчас ты в `deep flow` (BOM-refactor), активная нитка важнее любой почты — если письма не `high`, можешь сначала вернуть пользователю текущую нитку и спросить «обработать почту сейчас или после релиза».
